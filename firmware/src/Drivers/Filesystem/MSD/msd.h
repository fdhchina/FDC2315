...
#define MSD_SPI_NAME		1
#define MSD_SPI                       SPI1
#define MSD_SPI_APB			RCC_APB2Periph_SPI1
...
#if(GC_ZR&GC_ZR_GPIO)
#define MSD_CARDIN_PORT		GPIOC
#define MSD_CARDIN_GPIO_PORT_CLOCK	RCC_APB2Periph_GPIOC
#define MSD_CADRIN_PIN			GPIO_Pin_4
#else
#define MSD_CARDIN_PORT		GPIOA
#define MSD_CARDIN_GPIO_PORT_CLOCK	RCC_APB2Periph_GPIOA
#define MSD_CADRIN_PIN			GPIO_Pin_3
#endif
...
extern volatile BOOL sd_valid_flag;
BOOL	sd_IsInserted(void);				// SD卡插入否 

void sd_Config(void); //初始化IO

u8 SD_Init(void);                               //SD卡初始化
u32 SD_GetCapacity(void);                       //取SD卡容量

DSTATUS msd_disk_initialize (void);
DSTATUS msd_disk_status (void);
DRESULT msd_disk_read (BYTE*, DWORD, BYTE);
#if	_READONLY == 0
DRESULT msd_disk_write (const BYTE*, DWORD, BYTE);
#endif
DRESULT msd_disk_ioctl (BYTE, void*);
