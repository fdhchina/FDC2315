...
#define RST_LOW			(GPIOB->BRR=GPIO_Pin_5)
#define RST_HIGH		(GPIOB->BSRR=GPIO_Pin_5)

#define	SCLK_LOW		(GPIOB->BRR=GPIO_Pin_6)
#define SCLK_HIGH		(GPIOB->BSRR=GPIO_Pin_6)

#define SDIO_LOW		(GPIOB->BRR=GPIO_Pin_7)
#define SDIO_HIGH		(GPIOB->BSRR=GPIO_Pin_7)
...
{
	if(operation == READ)
		return I2C1_ReadBuffer(SI47XX_CHIPADDR, 0, 0, data1, numBytes);
	else 
		return I2C1_WriteBuffer(SI47XX_CHIPADDR, 0, 0, data1, numBytes);
