...
#else
#define KEY_OUT_LINE_NULL	GPIOC->BSRR=0x0f<<12		/*所有输出为高*/
#define KEY_OUT_LINE1		GPIOC->BRR=GPIO_Pin_12  // 0x01<<8	/*输出第一行*/
#define KEY_OUT_LINE2		GPIOC->BRR=GPIO_Pin_13	/*输出第二行*/
#define KEY_OUT_LINE3		GPIOC->BRR=GPIO_Pin_14	/*输出第三行*/
#define KEY_OUT_LINE4		GPIOC->BRR=GPIO_Pin_15	/*输出第四行*/
#define KEY_OUT_LINE_ALL	GPIOC->BRR=0x0f<<12		/*所有输出为低*/

#define KEY_IN				((GPIOC->IDR&(GPIO_Pin_8|GPIO_Pin_9|GPIO_Pin_10|GPIO_Pin_11))>>8)		/*获取输入直接移位到低端*/
#endif
...
{
	u8 ret = 0x80;
	u8 now_in = GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_12);
	u8 now_key = GPIO_ReadInputDataBit(GPIOE, GPIO_Pin_5);
